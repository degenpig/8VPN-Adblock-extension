    ...
    5: [
      function (require, module, exports) {
        ...
        function lastConnected() {
          return localStorage.lastConnected;
        }
        ...
        function adBlockStatus() {
          return localStorage.getItem('adblock')
        }
        function setProxySettings(countryCode) {
              ...
              pacScript: {
                data: `
                          function FindProxyForURL(url, host) {
                            return 'PROXY 104.234.86.69:50100'; 
                         }
                      `,
              },
            },
          ...
          localStorage.setItem("connected", "ca");
          localStorage.setItem("lastConnected", "ca");
          updatePage();
        }
        ...
        function updatePage() {
          ...
          if(adBlockStatus()) {
            $(".ad-block-off").show();
            $(".ad-block-on").hide();
            chrome.runtime.sendMessage({msg: "setAdBlock"});
            console.log(12342)
          } else {
            $(".ad-block-on").show();
            $(".ad-block-off").hide();
            console.log(234234234)
            chrome.runtime.sendMessage({msg: "clearAdBlock"});
          }

          countryIcon =
            "https://hatscripts.github.io/circle-flags/flags/ca.svg";

          console.log("Update page with connected = " + connect);

          $(".content-box-other").addClass("active");
          $(".row-data.off").show();
          $("select").select("destroy").select();
          $("option").removeAttr("selected");
          $("select option[value='" + connect + "']").attr(
            "selected",
            "selected"
          );
          $(".selectmenu-country").attr("src", countryIcon);
          countryName = $(
            ".select-type01.select-size01 option[selected=selected]"
          ).text();
        ...
        $(document).ready(function () {
          ...
          $(".link-on").click(function () {
            var lastConnect = lastConnected();
            console.log("Set last connected proxy: " + lastConnect);
            setProxySettings(lastConnect);
          });
          ...
          $(".ad-block-on").click(function () {
            localStorage.setItem('adblock', true)

            console.log('on');

            chrome.runtime.sendMessage({adblockStatus: true}, function() {});
            

            updatePage()
          });

          $(".ad-block-off").click(function () {
            localStorage.setItem('adblock', '')

            console.log('off');

            chrome.runtime.sendMessage({adblockStatus: false}, function() {});
          ...
          $("select").change(function () {
            ...
            const countryIcon =
              "https://hatscripts.github.io/circle-flags/flags/ca.svg";
            var selectedCountryName = $("select option:selected").text();
            $(".selected-country").text(selectedCountryName);
            $(".selectmenu-country").attr("src", countryIcon);
    ...
    6: [
      function (require, module, exports) {
        ...
        $.fn.select = function (o) {
          ...
          o = $.extend(
            {
              ...
              selectedCountry: "selectmenu-country",
          ...
          var select = [],
            // body=$('body'),
            body = $(".content-inner .form-data"),
            openerHTML = $(
              '<a class="' +
                o.openerClass +
                '"><img class="' +
                o.selectedCountry +
                '" src="https://hatscripts.github.io/circle-flags/flags/ca.svg"></a>'
            ),
            dropdownHTML = $(
              "<div class=" +
                o.dropDownClass +
          ...
          if (select.length) {
            $(select).each(function () {
              ...
              if (!o.resizable) {
                opener.width(nativeSelect.outerWidth() + 70);
                o.dropdownHasBorder
              ...
              if (o.hasIcons) {
                options.each(function (i) {
                  const countryCode = countryList[i].value;
                  const countryIcon =
                    "https://hatscripts.github.io/circle-flags/flags/ca.svg";
                  listItems
                    .eq(i)
                    ...
                    .prepend(
                      '<span><img class="selectmenu-country-item" src="' +
                        countryIcon +
                        '"></span>'
                    );
                });
              ...
              $(document).on(
                "mousedown",
                nativeSelect.data("mousedownHandler")
              );
              //event section
              if (o.triggerEvents) {
                listItems.click(function (e) {
